import type { CardsDashboardProps } from './cards-dashboard.types';

export function CardsDashboard({ state }: CardsDashboardProps) {
  return (
    <article className="home-layout__dashboard">
      ...
      {state.isLoading ? <p>Carregando cartões...</p> : null}
      {state.hasError ? <p>Não foi possível carregar cartões.</p> : null}
    </article>
  );
}
